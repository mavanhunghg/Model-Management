...
| status         | ModelStatus (enum) | DEFAULT INACTIVE                 | ACTIVE / INACTIVE / DEPRECATED            |
...
| Cột Status        | Badge màu: Xanh=ACTIVE, Xám=INACTIVE, Đỏ=DEPRECATED                                    | Nhận biết trạng thái nhanh     |
| Cột Thao tác      | 3 nút: Sửa (icon bút) / Kích hoạt-Toggle (icon lightning) / Xóa (icon thùng rác)       | Hành động trực tiếp trên hàng  |
...
### **4.1.3. Nút Kích Hoạt và Xóa**

- **Nút "Kích hoạt":** Màu xanh lá với icon ⚡ khi mô hình INACTIVE. Khi đang ACTIVE, chuyển thành nút "Vô hiệu hóa" màu cam. Tooltip hiển thị hành động sẽ thực hiện.
- **Nút "Xóa":** Màu đỏ; bị disabled (xám, không click được) nếu is_active=true. Khi click, hiện Confirmation Dialog: "Bạn có chắc muốn xóa mô hình \[tên\]? Hành động này không thể hoàn tác."
